...
def main() -> None:
    outbase = Path("out/hf_scan_closed_shells")
    outbase.mkdir(parents=True, exist_ok=True)

    # Mesh: OK for O-16/Ca; for Pb-208 you’ll want larger r_max and n_points.
    mesh = RadialMesh.uniform(r_max_fm=20.0, n_points=300)
    sk = get_skyrme("SLy4")

    # Keep the same "calibrated placeholder" settings you used for O-16
    C_SURF = 300.0
    W0 = 120.0
    ...
    nuclei = [
        ...
        ("ca40", 20, 20),
        ("ca48", 20, 28),
        # ("pb208", 82, 126),  # later: increase mesh + l_max + n_states_per_l
    ]

    for tag, Z, N in nuclei:
        outdir = outbase / tag
        ...
        res = run_hf_minimal_t0t3(
            ...
            nucleus_N=N,
            skyrme=sk,
            l_max=8,              # Ca needs a bit more than O-16
            n_states_per_l=12,    # enough basis states to fill
            max_iter=600,
            mixing=0.05,
            tol_rho=2e-5,
            C_surf=C_SURF,
        ...
        print("\nNeutron orbitals (occupied):")
        for o in res.orbitals_n:
            if o.occ > 0:
                print(fmt_orb(o))

        print("\nProton orbitals (occupied):")
        ...
        # If you implemented "clean" saving via HFMinimalResult:
        if hasattr(res, "U_n"):
            np.save(outdir / "U_n_MeV.npy", res.U_n)
            np.save(outdir / "U_p_MeV.npy", res.U_p)
        if getattr(res, "Uls_n", None) is not None:
            np.save(outdir / "Uls_n_MeV.npy", res.Uls_n)
        if getattr(res, "Uls_p", None) is not None:
